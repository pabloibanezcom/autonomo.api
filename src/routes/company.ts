...
const registerCompanyRoutes = (router: express.Router): void => {
  router.get(routePaths.COMPANY, (req: Request, res: Response) => {
    getCompanies()
  ...
  router.get(`${routePaths.COMPANY}/:id`, (req: Request, res: Response) => {
    getCompany(req.params.id)
      .then((result: Company) => res.status(httpCode.GET).send(result))
      .catch((err: HttpError) => res.status(err.code || transformGenericError(err).code).send(err.message));
  });

  router.post(routePaths.COMPANY, (req: Request, res: Response) => {
    addCompany(req.body)
      .then((result: Company) => res.status(httpCode.POST).send(result))
      .catch((err: HttpError) => res.status(err.code || transformGenericError(err).code).send(err.message));
  });

  router.put(`${routePaths.COMPANY}/:id`, (req: Request, res: Response) => {
    updateCompany(req.params.id, req.body)
      .then((result: Company) => res.status(httpCode.PUT).send(result))
      .catch((err: HttpError) => res.status(err.code || transformGenericError(err).code).send(err.message));
  });

  router.delete(`${routePaths.COMPANY}/:id`, (req: Request, res: Response) => {
